-- Update wallet_topup RPC to include authorization check using auth.uid()
-- This adds defense-in-depth by verifying the caller is the wallet owner
...
DECLARE
  ...
  _calling_user UUID;
BEGIN
  -- Get the calling user's ID (will be set when called with user's auth context)
  _calling_user := auth.uid();
  
  -- Authorization check: ensure the caller is the wallet owner
  -- This provides defense-in-depth even though the edge function validates this too
  IF _calling_user IS NOT NULL AND _calling_user != _user_id THEN
    RAISE EXCEPTION 'Unauthorized: cannot top up other users wallets';
  END IF;
